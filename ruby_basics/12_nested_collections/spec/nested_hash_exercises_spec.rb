...
RSpec.describe 'Nested hash exercises' do
  describe 'find language information exercise' do
    ...
    context 'when the given language name is coldfusion' do
      # remove the 'x' from the line below to unskip the test
      it 'returns the information for coldfusion' do
        #expected_output = { initial_release: 'July 2, 1995', is_beautiful?: false }
        expect(find_language_information(hash, :coldfusion)).to eq(nil)
      end
    end
end
